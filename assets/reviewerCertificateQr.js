/*
 * Reviewer Certificate local QR renderer.
 * QR encoder: Copyright (c) 2009 Kazuhiko Arase, MIT License.
 * Browser integration: Copyright (c) 2026 Mohammad Fauziddin.
...
module.exports = QRCode;
	...
	var ErrorCorrectLevel = localRequire('./QRErrorCorrectLevel');
	...
	function createSvg(text, options) {
		options = options || {};
		var quietZone = Math.max(2, parseInt(options.quietZone || 4, 10));
		var qr = new QRCode(-1, ErrorCorrectLevel.M);
		...
		return '<svg xmlns="http://www.w3.org/2000/svg" role="img" aria-label="' +
			escapeXml(options.label || 'Certificate verification QR code') +
			'" viewBox="0 0 ' + totalSize + ' ' + totalSize +
			'" shape-rendering="crispEdges"><rect width="100%" height="100%" fill="#fff"/>' +
			'<path d="' + pathData.join('') + '" fill="#000"/></svg>';
	}

	function render(target, text, options) {
		var element = typeof target === 'string' ? document.getElementById(target) : target;
		if (!element) throw new Error('QR target was not found.');
		element.innerHTML = createSvg(text, options);
		return element.querySelector('svg');
	}

	var api = {createSvg: createSvg, render: render};
	root.ReviewerCertificateQr = api;
